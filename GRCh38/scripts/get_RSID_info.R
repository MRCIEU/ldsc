...
write.table(coordinates,
            file = "../data/hm3/rsid.coordinates.tsv",
            quote = F,
            sep = "\t",
            row.names = F)
...
for(chr in 1:22) {
  ...
  temp <- coordinates[idx,]
  temp$CHROM <- sapply(temp$CHROM, function(x) { paste0("chr", x) } )
  write.table(temp,
              file = paste0("../data/hm3/chr", as.character(chr), ".rsid.coordinates.tsv"),
              quote = F,
              sep = "\t",
              col.names = T,
              row.names = F)
  write.table(temp[, 1:2],
              file = paste0("../data/hm3/chr", as.character(chr), ".rsid.coordinates.regions"),
              quote = F,
              sep = "\t",
              row.names = F,
              col.names = F)
